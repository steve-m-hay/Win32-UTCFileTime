...
C<utime(2)> may change by an hour as we move into or out of DST if the computer
is set to "Automatically adjust clock for daylight saving changes" (which is the
default setting) and the file is stored on an NTFS volume (which is the
preferred filesystem used by Windows NT/2000/XP).
...
due to the fact that the mapping from UTC to local time is not one-to-one. It is
straightforward to convert UTC to local time, but there is an ambiguity when
converting back from local time to UTC involving DST. The Win32 API provides two
documented functions (C<FileTimeToLocalFileTime()> and
...
an argument must be supplied - it does not default to C<$_> like the built-in
functions' arguments do;

=item *

the argument must be a file (path or name), not a filehandle (and not the
special filehandle consisting of an underscore character either).
...
[C<utime()> only.] One or more of the I<@files> is read-only. (The process must
have write access to each file to be able to change its last access time or last
modification time.)
...
times. This decision is documented in the Knowledge Base in articles Q128126 and
...
times, which FAT does not record) in UTC. The first question you may want to ask
is, "How should Windows report these file times?" Clearly it would be stupid for
C<dir> and Windows Explorer to report FAT file times in the local time zone and
NTFS file times in UTC. If inconsistent formats were used, users would have
great difficulty determining which of two files was more recent. We must thus
choose to translate one of the two file time formats when we report to the user.
Most users are likely to want to know the file modification time in their local
time zone. This keeps things consistent with what people learned to expect under
DOS and Win16. It also is more useful to most users, who may want to know how
long ago they modified a file without looking up the offset of their local time
zone from UTC.

It is straightforward to translate UTC to local time. You look up the offset, in
minutes, between the local time zone and UTC, determine whether daylight savings
is in effect and add either the standard or the daylight offset to the UTC time.
However, daylight time throws a subtle wrench in the works if we try to go
backwards...
...
time to arrive at UTC. A subtle problem emerges due to the fact that the mapping
...
There is a bit of a chicken-and-egg problem here. Windows does not supply a good
API call to let you determine whether DST was in effect at a given time.
